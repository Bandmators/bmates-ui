...
## Note about generated styles

This repository previously committed the `styled-system/` directory, which is generated by Panda CSS.
The generated output is no longer tracked in the repository and can be recreated when needed.

To regenerate the `styled-system` output locally or in CI, run:

```bash
pnpm exec panda codegen
pnpm exec panda cssgen --clean
```

Make sure to include `pnpm run panda:watch` during development if you want automatic regeneration.
